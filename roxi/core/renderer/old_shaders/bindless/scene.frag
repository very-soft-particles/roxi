...
layout(set = 0, binding = 4) uniform samplerCube specular_map;
layout(set = 0, binding = 5) uniform samplerCube diffuse_map;
layout(set = 0, binding = 6) uniform sampler2D brdf_lut;

// NOTE This requires descriptor indexing feature
layout(set = 0, binding = 7) uniform sampler2D pbr_textures[];
...
void main()
{
	...
	vec3 texNormalValue = texture(pbr_textures[nonuniformEXT(mesh_data.normal)], tex_coord).xyz * 2.0 - 1.0;
	...
	// Input lighting data
	vec3 n = normal_tbn(texNormalValue, world_pos, normal, tex_coord);
