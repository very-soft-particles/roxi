...
namespace roxi {
  ...
  b8 ClusteredForwardRenderer::init() {
    ...
    for(u32 i = 0; i < RoxiNumFrames; i++) {
      buffer[i].buffer.type = gpu::BufferType::HostUniformBuffer;
      buffer[i].buffer.size = CameraSize;

      buffer[RoxiNumFrames + i].buffer.type = gpu::BufferType::DeviceUniformBuffer;
      buffer[RoxiNumFrames + i].buffer.size = DrawParamsSize;

      buffer[(2 * RoxiNumFrames) + i].buffer.type = gpu::BufferType::DeviceStorageBuffer;
      buffer[(2 * RoxiNumFrames) + i].buffer.size = AABBsSize;

      buffer[(3 * RoxiNumFrames) + i].buffer.type = gpu::BufferType::DeviceStorageBuffer;
      buffer[(3 * RoxiNumFrames) + i].buffer.size = LightCellsSize;

      buffer[(4 * RoxiNumFrames) + i].buffer.type = gpu::BufferType::DeviceStorageBuffer;
      buffer[(4 * RoxiNumFrames) + i].buffer.size = LightIndicesSize;

      buffer[(5 * RoxiNumFrames) + i].buffer.type = gpu::BufferType::DeviceStorageBuffer;
      buffer[(5 * RoxiNumFrames) + i].buffer.size = LightDataSize;
 
      buffer[(6 * RoxiNumFrames) + i].buffer.type = gpu::BufferType::DeviceStorageBuffer;
      buffer[(6 * RoxiNumFrames) + i].buffer.size = InstanceDataSize;   

      buffer[(7 * RoxiNumFrames) + i].buffer.type = gpu::BufferType::DeviceStorageBuffer;
      buffer[(7 * RoxiNumFrames) + i].buffer.size = IndirectCommandsSize;
    }

    buffer[(8 * RoxiNumFrames)].buffer.type = gpu::BufferType::DeviceStorageBuffer;
    buffer[(8 * RoxiNumFrames)].buffer.size = MeshesSize;

    buffer[(8 * RoxiNumFrames) + 1].buffer.type = gpu::BufferType::DeviceStorageBuffer;
    buffer[(8 * RoxiNumFrames) + 1].buffer.size = VerticesSize;

    buffer[(8 * RoxiNumFrames) + 2].buffer.type = gpu::BufferType::DeviceStorageBuffer;
    ...
    _command_arenas_begin = get_command_pool().create_command_arenas(RoxiNumFrames * RoxiNumThreads, 2);

    const vk::CommandBuffer command_buffer = get_command_pool().obtain_command_arena(_command_arenas_begin).obtain_primary_command_buffer();
